...
module.exports = (self, { scanRules }) => {
  ...
  // self.checkIfDeadLinks = async ({
  //   $, url, seoResults, cookie
  // }) => {
  //   const $links = $('a');
  ...
  //       const linkToBrowse = isInternalLink
  //         ? `${url.endsWith('/') ? url.slice(0, -1) : url}${link}`
  //         : link;

  //       // We add the cookie only for internal links to avoid errors
  //       // if a piece, for example, is draft but not live,
  //       // with the cookie, apostrophe will get the draft page
