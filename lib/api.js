  ...
  self.checkContentSize = ($) => {
    ...
    if (totalWords < minWordsNumber) {
      return {
        ...
        which does not meet the minimum requirements of <strong>${minWordsNumber}</strong>.`
      };
    }
    return {
      error: false,
      msg: `There are <strong>${totalWords}</strong> words on the page, which meets the minimum requirements of <strong>${minWordsNumber}</strong>.`
